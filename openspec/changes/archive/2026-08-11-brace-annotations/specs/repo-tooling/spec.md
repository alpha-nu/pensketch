...
> A phase costs bytes in the entry that carries its own copy of `draw`. The
> budget moves once, before the phase is written, rather than a byte at a time
> at each gate.
...
bytes, `@pensketch/core/server` exceeds 3648 bytes, or `@pensketch/react`
