...
that added it.
- [x] 5.3 **OWNER**: dispatch `release.yml` twice. This is the batched release
      ...
      Merged as "Version Packages (#3)", carrying four changesets rather than
      three: `react-incident-example` was written after this list and rode the
      same pull request. `@pensketch/core` and `@pensketch/mcp` are 0.2.0;
      `@pensketch/react` stayed at 0.1.0, its source untouched and its peer
      range on core already `>=0.0.1 <1.0.0`
- [x] 5.4 **OWNER**: draw a brace through the MCP server in a real client

      Drawn in a client by the owner. Corroborated independently against
      `@pensketch/mcp@0.2.0` installed from the registry: a diagram carrying a
      `braces` array rendered an `<svg>` of 24 paths over stdio — the field
      that could not cross a JSON boundary at all when this change was
      written
