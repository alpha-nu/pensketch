...
> One number moves. `@pensketch/core/check` goes from 3392 to 3520 bytes,
...
> Restated from the live baseline in full, with all five of its scenarios carried
> word for word and none added. The only edit is `3392` to `3520` in the first
> sentence; every other clause — the self-contained-entry rule, the README
> comparison gate, the `@pensketch/mcp` exemption, and the paragraph requiring
> a budget to move before the code — is unchanged, that last one being the
> clause this change is obeying by moving it now.
