...
- [x] 7.2 **OWNER**: the dispatch is batched with `strict-tool-input` and
      ...
      Went out in the batch. `@pensketch/core` and `@pensketch/mcp` are
      **0.2.0** on the registry, carried by "Version Packages (#3)"
- [x] 7.3 **OWNER**: after publishing, draw a self-transition through the MCP
      server in a real client — the thing that was impossible when this change
      was written

      Drawn in a client by the owner. Corroborated independently against the
      published tarball: `@pensketch/mcp@0.2.0` installed from the registry and
      spawned over stdio the way a client spawns it, `render_diagram` with an
      edge naming `['pin','r']` at both ends returned an `<svg>` of 14 paths.
      The thing this change was written for is a round trip a stranger can
      make
