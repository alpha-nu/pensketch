...
- [x] 1.3 Edge path derivation from the exported `anchor` plus `via`, with a
      test asserting the path matches what `draw` renders for a known edge —
      the two must not drift
- [x] 1.4 Segment inflation by `AMP / 2 + WIDTH / 2`, read from `constants`
      rather than re-declared, with a test that fails if either constant moves
      and the checker does not follow

## 2. The rules

- [x] 2.1 `duplicate-id` and `orphan-node` — pure id analysis, no geometry
- [x] 2.2 `node-overlap` and `group-escape`, including the case that must
      *not* fire: a node wholly outside a group
- [x] 2.3 `out-of-bounds`, and the documented behaviour when no `viewBox` is
      supplied
- [x] 2.4 `text-overflow` with the D3 estimate, `estimated: true` on the
      finding, and a test pinning the default `glyphWidth` against the
      measured table
- [x] 2.5 `label-collision` over edge labels and note text, with a regression
...
- [x] 2.6 Severity overrides and `off`, and the stable sort. A test asserting
      two runs are deeply equal, and one asserting sort order does not depend
      on rule evaluation order

## 3. Packaging

- [x] 3.1 `src/check.ts` added to tsup `entry`; `exports` gains `./check` with
      the nested `types`/`import`/`require` shape
- [x] 3.2 `tools/check-size.mjs` gains a budget for the subpath, and the
      existing core budget is asserted unchanged
- [x] 3.3 Resolution test: import the built subpath as ESM and require it as
      CJS, asserting both expose exactly the D1 surface — the same treatment
      the root entry already gets
- [x] 3.4 Coverage thresholds extended to the new source, at the same 90%
      lines and branches

## 4. First-party use and documentation

- [x] 4.1 `tools/check-diagrams.mjs` running the checker over every diagram
      this repository ships — the three examples and the hero — exiting
      non-zero on any `error`
- [x] 4.2 Wire it into CI after the size check, and into the `CONTRIBUTING.md`
      verification list
- [x] 4.3 Fix whatever it finds in the shipped diagrams. Findings here are
      binding: the rules are not credible if the project shipping them fails
      its own
- [x] 4.4 README section documenting `check`, the rule table, and the
      estimation caveat; `packages/core/README.md` likewise
- [x] 4.5 A changeset: **minor** on `@pensketch/core`. New API, no rendered
