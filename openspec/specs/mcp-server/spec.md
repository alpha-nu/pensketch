...
The transport SHALL be published as two entries, not one. The web-standard
`fetch` handler SHALL reach no Node built-in, so that a Worker, a Deno Deploy
project or a Bun deployment can import it; the listener that binds a socket
SHALL live in a separate entry, with the Node imports that binding one needs.
A single entry holding both puts `node:http` - and `http2`, `stream` and
`crypto` through the SDK's Node adapter - into the graph of every consumer,
which no edge runtime can load.

That property SHALL be asserted by bundling the handler for a runtime with no
Node built-ins, rather than by reading its imports. Two changes have broken it
without changing how the source reads: an entry that held both halves, and a
rasterizer moved behind a dynamic import, which a bundler follows and resolves
exactly as it would a static one.
...
#### Scenario: The handler an edge runtime imports
- **WHEN** the `fetch` handler entry is bundled for a target with no Node built-ins
- **THEN** it bundles, and serving is proved by listing its tools rather than by the bundle merely being produced
...
client's latency in a process that serves more than one. It also loads its
WebAssembly off the filesystem, which an edge runtime does not have.

Unreachable SHALL mean unreachable in the module graph, not switched off by a
flag. The tool set SHALL be decided by which factory a transport calls: one
building the pair every transport serves, and one adding the rasterizer for a
transport that owns its process. A boolean passed to a single factory leaves
the rasterizer imported by both, which is what made the handler unloadable on
every edge runtime while reading as though it had been excluded.
