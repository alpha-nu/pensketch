...
- **The core is tiny and dependency-free.** About 5.5 KB minified and gzipped,
...
```sh
claude mcp add pensketch -- npx -y @pensketch/mcp@0.12.0
```
...
nothing drawn under it, and why this picture can draw itself in front of you:
the animation above is that stylesheet at work, two calls and no keyframe of
anyone's own. Below them core, one package with four entry points;
...
passes outright, and drawing that same diagram emits 6,388
paths and 1.2 MB of markup. Depth costs about 56 B per px on top of
...
same bytes on every run, for about 5.5 KB and no new entries in your lockfile.
