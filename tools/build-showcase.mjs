...
/* The assistant: a launcher, and the box a chat widget mounts into.
   ...
   Two pieces and no more. The embed takes its size, its type and its
   palette from the box, so all three are declared here and nothing about
   the conversation is - what sits below the panel header is an empty
   state, and the widget replaces it on mount. */

/* The launcher rides the same right edge as the way out to the source, one
   glyph below it. Both marks are 39px, so 65px between centres is one
...
.chat-launch:hover, .chat-launch:focus-visible { color: var(--ps-accent); opacity: 1; }
...
.chat-scrim {
  ...
  padding: 0;
  border: 0;
  background: radial-gradient(40rem 38rem at 50% 50%,
    var(--scrim) 0%,
    var(--scrim) 48%,
    color-mix(in srgb, var(--scrim) 40%, transparent) 72%,
    transparent 100%);
  opacity: 0;
...
.chat-head .mark { display: block; width: 15px; height: 15px; fill: var(--ps-accent); }
.chat-close {
  display: block;
  margin: -6px;
  padding: 6px;
...
.chat-close:hover, .chat-close:focus-visible { color: var(--ps-accent); opacity: 1; }
.chat-close svg {
  display: block;
  width: 15px;
  height: 15px;
...
/* What the embed is handed: the rest of the panel as a column, its own
   scroll, and the page's serif and palette to inherit. */
.chat-mount {
  display: flex;
  flex-direction: column;
  flex: 1 1 auto;
...
/* The empty state: the widget's box at rest, with a line of invitation
   where the conversation will be and a composer where the composer will
   be, so the proportions read before there is anything to say. The bar is
   a drawing of a control, not a control, and is hidden from anything that
   would offer it as one. */
.chat-idle { display: flex; flex-direction: column; height: 100%; }
.chat-idle .greet {
  display: flex;
  flex: 1 1 auto;
  flex-direction: column;
...
.chat-idle .greet svg { display: block; width: 34px; height: 34px; fill: var(--ps-accent); opacity: 0.5; }
.chat-idle .greet p { max-width: 34ch; margin: 0; color: var(--ps-muted); }
.chat-compose {
  display: flex;
  align-items: center;
  gap: 12px;
...
@media (max-width: 720px) {
  .chat-panel { width: calc(100vw - 20px); height: calc(100svh - 40px); }
}
...
<dialog class="chat-scrim" aria-labelledby="chat-title">
  <div class="chat-panel">
    ...
    <!-- Where the embed mounts. It inherits this box - size, surface, type
         and palette - and replaces the empty state inside it. -->
    <div class="chat-mount" id="pensketch-chat">
      <div class="chat-idle">
        <div class="greet">
          ${SPARK('')}
          <p>Ask about the data model, the draw options, or any figure on this page.</p>
        </div>
        <div class="chat-compose" aria-hidden="true">
          <span>Message pensketch</span>
          <span class="send"><svg viewBox="0 0 16 16" aria-hidden="true"><path d="M8 12.6 L8 3.9 M4.3 7.6 L8 3.9 L11.7 7.6" stroke-linecap="round" stroke-linejoin="round"/></svg></span>
        </div>
      </div>
    </div>
  </div>
