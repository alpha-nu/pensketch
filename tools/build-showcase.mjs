...
/* The shortcut for the reader who needs no show: muted, mono, and a jump
   to the closing slide. Smooth only for readers who have not asked
   otherwise - a fragment jump under mandatory snap lands on the slide
   either way. */
.skip { margin: 18px 0 0; font: 400 13px/1.6 var(--mono); }
.skip a { color: var(--ps-muted); }
...
#install .comment { color: var(--ps-muted); }
...
<header class="top">
  ...
  <p class="skip"><a href="#install">Already sold? Skip to the install line.</a></p>
...
<section class="fig" id="install">
  ...
  <pre><code><span class="comment"># the full server for a coding agent: check, render, and a PNG it can look at</span>
