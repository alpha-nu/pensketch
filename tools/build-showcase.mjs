...
/* The shade, and it is the panel's own shape rather than a circle. A
   radial gradient is an ellipse by definition, so a round halo around a
   square box was the only thing it could have drawn; what makes this
   rectangular is that it *is* a rectangle, blurred. The box runs 40px
   past the panel on every side so that full strength reaches the edge
   rather than the falloff starting under it, and the blur then carries it
   out another 100px or so before it is gone. */
.chat-scrim::before {
  content: '';
  position: absolute;
  top: 50%;
  left: 50%;
  width: calc(var(--panel-w) + 80px);
  height: calc(var(--panel-h) + 80px);
...
(async () => {
  ...
  const applyOverrides = () => {
    ...
    widget.overrides = {
      ...
      // roughly 2:1 and was the one illegible thing in the panel. No token
