...
**The cost is linear and nothing bounds it.** About **56 B per px** of depth
(least squares over depths 100 to 1000 on one 150 × 46 box at 40, 40, seed
7 — the figures reproduce only at that position), on top
of a fixed **2,920 B** for the faces themselves, which is what the markup
grows by as the depth approaches nought. One box at `depth: 1000` renders 60 kB. No rule caps a depth, and `check` reads its form
